...
resource "azurerm_resource_group" "main" {
  name     = "rg-agentbridge-${var.environment}-engagement-personalization-with-ott-plat"
  location = var.region
  tags = {
...
resource "azurerm_virtual_network" "main" {
  name                = "vnet-agentbridge-engagement-personalization-with-ott-plat"
...
resource "azurerm_subnet" "main" {
  name                 = "snet-agentbridge-engagement-personalization-with-ott-plat"
  resource_group_name  = azurerm_resource_group.main.name
...
resource "azurerm_network_security_group" "main" {
  name                = "nsg-agentbridge-engagement-personalization-with-ott-plat"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_public_ip" "main" {
  ...
  name                = "pip-agentbridge-engagement-personalization-with-ott-plat"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_network_interface" "main" {
  name                = "nic-agentbridge-engagement-personalization-with-ott-plat"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_linux_virtual_machine" "main" {
  name                  = "vm-agentbridge-engagement-personalization-with-ott-plat"
  resource_group_name   = azurerm_resource_group.main.name
  location              = azurerm_resource_group.main.location
...
resource "azurerm_managed_disk" "data" {
  ...
  name                 = "disk-agentbridge-engagement-personalization-with-ott-plat-data"
  location             = azurerm_resource_group.main.location
  resource_group_name  = azurerm_resource_group.main.name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_cognitive_account" "ai_services" {
  ...
  name                  = "agentbridge-engagement-personalization-with-ott-plat-ai"
  location              = azurerm_resource_group.main.location
  resource_group_name   = azurerm_resource_group.main.name
  kind                  = "AIServices"
  sku_name              = "S0"
  custom_subdomain_name = "agentbridge-engagement-personalization-with-ott-plat-ai"
...
resource "azurerm_user_assigned_identity" "vm_foundry" {
  count               = var.enable_foundry ? 1 : 0
  name                = "agentbridge-engagement-personalization-with-ott-plat-vm-identity"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
}
...
resource "null_resource" "foundry_agents" {
  ...
  provisioner "local-exec" {
    interpreter = ["bash", "-c"]
    command     = <<-EOT
      ENDPOINT="https://agentbridge-engagement-personalization-with-ott-plat-ai.services.ai.azure.com"
